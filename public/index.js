...
function createTab(location, firstLoad) {
  ...
  if (index == -1) {
    ...
    tab.oncontextmenu = () => {
      oldPaths = oldPaths.filter((el) => {
        return el != location;
      });
      document.getElementById("tabs").removeChild(tab);
      if (
        oldPaths.length != 0 &&
        isExist(location, oldPaths) === oldPaths.length - 2
      ) {
...
function openFile(location, firstLoad) {
  if (fs.existsSync(location)) {
    ...
    ipcRenderer.send(
      "setWindowTitle",
      path.basename(location) + " - Ceres Editor"
    );
  } else {
    editor.setValue("");
  }
  ...
  if (localStorage.getItem("folder") != null) {
    ...
    if (input.value.length > 1) {
      if (input.value === ui.commands.dialogExit) {
        ...
        if (
          fs.existsSync(path.join(localStorage.getItem("folder"), input.value))
        ) {
          alert(
            "File/folder exists. Please change location or the file/folder name."
          );
        } else {
          document.querySelector(".inputDialog").classList.remove("visible");
          try {
            if (dialogType === "file") {
              fs.writeFileSync(
                path.join(localStorage.getItem("folder"), input.value),
                ""
              );
